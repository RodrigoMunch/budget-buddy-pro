...
import { LayoutDashboard, Tags, User, LogOut, DollarSign, CreditCard, History, Shield } from "lucide-react";
...
const AppLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <aside className={`w-64 gradient-dark text-primary-foreground flex flex-col fixed h-full z-10 max-md:hidden ${impersonating ? "mt-8" : ""}`}>
      ...
      {/* Mobile header */}
      <div className={`md:hidden fixed top-0 left-0 right-0 z-10 gradient-dark ${impersonating ? "mt-8" : ""}`}>
        ...
        <nav className="flex gap-1 px-3 pb-3">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.to} to={item.to}
                className={`flex-1 flex flex-col items-center gap-1 py-2 rounded-lg text-xs font-medium transition-colors ${isActive ? "gradient-primary text-primary-foreground" : "text-primary-foreground/60"}`}>
      ...
      <main className={`flex-1 md:ml-64 p-6 max-md:pt-28 ${impersonating ? "mt-8" : ""}`}>{children}</main>
    </div>
  );
};
