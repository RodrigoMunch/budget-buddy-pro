...
const InstallmentsPage = () => {
  ...
  // Group installment transactions by parentId
  const installmentGroups = new Map<string, typeof transactions>();
  transactions.forEach((t) => {
    if (t.parentId && t.installments && t.installments > 1) {
      const group = installmentGroups.get(t.parentId) || [];
      group.push(t);
      installmentGroups.set(t.parentId, group);
    }
  });

  // Determine projection range: from current month to the furthest installment month
  const now = new Date();
  ...
  while (cursor <= startOfMonth(maxDate)) {
  ...
  const monthlyTotals = months.map((month) => {
    ...
    installmentGroups.forEach((group) => {
      group.forEach((t) => {
        if (isSameMonth(new Date(t.date), month)) total += t.amount;
      });
    });
    return total;
  });
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm">Projeção de parcelas futuras</p>
            </div>
          </div>
        </motion.div>
        ...
        {groupEntries.length === 0 ? (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            <Card>
              <CardContent className="py-16 text-center">
                ...
                <p className="text-muted-foreground">Nenhum parcelamento encontrado</p>
                <p className="text-sm text-muted-foreground/60 mt-1">Registre uma despesa parcelada no Dashboard</p>
              </CardContent>
            </Card>
          </motion.div>
        ) : (
          <>
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="grid gap-4 md:grid-cols-3">
              ...
              <Card>
                <CardContent className="p-5">
                  <p className="text-sm text-muted-foreground mb-1">Total Este Mês</p>
                  <p className="text-2xl font-bold text-destructive">R$ {monthlyTotals[0]?.toFixed(2) || "0.00"}</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-5">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <p className="text-sm text-muted-foreground">Projeção até</p>
                  </div>
                  <p className="text-2xl font-bold capitalize">
                    {format(maxDate, "MMM/yyyy", { locale: ptBR })}
                  </p>
                </CardContent>
              </Card>
            </motion.div>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        {months.map((m) => (
                          <TableHead key={m.toISOString()} className="min-w-[110px] text-right capitalize">
                    ...
                    <TableBody>
                      {groupEntries.map(([parentId, group]) => {
                        const sorted = [...group].sort((a, b) => (a.currentInstallment || 0) - (b.currentInstallment || 0));
                          ...
                          <TableRow key={parentId}>
                            <TableCell className="font-medium sticky left-0 bg-card z-10">
                              <div className="flex items-center gap-2">
                                <span>{getCategoryIcon(first.categoryId)}</span>
                                <span>{first.description}</span>
                              </div>
                            </TableCell>
                            <TableCell>
                              <Badge variant="secondary" className="text-xs">{getCategoryName(first.categoryId)}</Badge>
                            </TableCell>
                            <TableCell className="text-center">
                              <span className="text-sm">{paid}/{totalInstallments}</span>
                            </TableCell>
                            {months.map((month) => {
                                ...
                                <TableCell key={month.toISOString()} className="text-right">
                                  {match ? (
                                    <span className={`text-sm font-medium ${
                                      isPast || isSameMonth(month, now) && new Date(match.date) <= now
                                        ? "text-muted-foreground line-through"
                                        : "text-destructive"
                                    }`}>
                                      R$ {match.amount.toFixed(2)}
                                      <span className="text-xs text-muted-foreground ml-1">
                                        ({match.currentInstallment}/{totalInstallments})
                                      </span>
                                    </span>
                                  ) : (
                      ...
                      <TableRow className="font-bold border-t-2">
                        ...
                        {monthlyTotals.map((total, i) => (
                          <TableCell key={i} className="text-right text-destructive">
                            R$ {total.toFixed(2)}
                          </TableCell>
                        ))}
                      </TableRow>
